...
async def register_user(request: Request,data: UserRegisterScheme, db: SessionDep) -> User:
...
async def login_user(response: Response,request: Request, data: UserLoginScheme, db: SessionDep) -> dict:
    token = await auth_user(data=data, session=db)
    response.set_cookie(value=token,key="access_token",secure=True, httponly=True, samesite="lax")
    return {"status": "ok"}
...
async def forgot_password_handler(request: Request,data: UserPasswordForgot, db: SessionDep) -> dict:
...
@limiter.limit("1/minute")
async def reset_password_handler(request: Request,data: UserPasswordReset, db: SessionDep) -> dict:
